...
declare function local:createSpringBean(
 ...
 declare function local:getBeanClassName($bean as element()) as xs:string
{
       if ( $bean/@class ) then $bean/@class else ""
};
...
declare function local:createProperties( $bean as element(), $property as element() ) as xs:string
{
	let $beanName := (local:getBeanName($bean))
    let $beanClassName := if ( $bean/@class ) then $bean/@class else ""
		return concat ( 'createSpringBeanProperty ("',
						string-join( ( $beanName, $beanClassName, $property/@name ), '", "' ), 
						'");',
						castbasics:CR())
};
declare function local:createPropertiesEmbedded($bean as element(), $refprop as xs:string) as xs:string
{
	let $beanName := (local:getBeanName($bean))
    let $beanClassName := if ( $bean/@class ) then $bean/@class else ""
		return concat ( 'createSpringBeanProperty("',
    string-join( ( $beanName, $beanClassName, $refprop ), '", "' ), 
						'");',
						castbasics:CR())
};
...
for $utilBean in /*:beans/*:bean/*:property/util:constant
return local:createUtilConstantBean( $utilBean ),

(: creation of Spring bean properties used in dependency injection :)
for $property in /*:beans/*:bean/*:property
where not(empty($property/@name)) or ( not(empty($property/*:ref/@bean)) or not(empty($property/*:ref/@local)) )
return local:createProperties($property/.., $property),

for $property in /*:beans/*:bean/@*[namespace-uri()='http://www.springframework.org/schema/p']
where substring-before(local-name($property), '-ref') != "" 
return local:createPropertiesEmbedded($property/.., substring-before(local-name($property), '-ref')),
