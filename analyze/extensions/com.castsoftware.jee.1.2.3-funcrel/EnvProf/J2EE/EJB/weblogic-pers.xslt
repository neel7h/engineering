		...
		<xsl:element name="ejb-module-infos">
			<xsl:element name="module-pers-infos">
				<xsl:for-each select="weblogic-ejb-jar/weblogic-enterprise-bean[entity-descriptor]">
					<xsl:variable name="persId"      select="entity-descriptor/persistence/persistence-type/type-identifier"/>
					<xsl:variable name="persVers"    select="entity-descriptor/persistence/persistence-type/type-version"/>
					<xsl:variable name="persStorage" select="entity-descriptor/persistence/persistence-type/type-storage"/>
					<xsl:element name="entity">
						...
						<xsl:element name="tables">
							...
							<xsl:if test="string($persId) = 'WebLogic_CMP_RDBMS'">
								...
								<xsl:for-each select="document(concat(string($sModuleDir), '/', string($persStorage) ))/weblogic-rdbms-jar/weblogic-rdbms-bean[ejb-name = $ejbName]">
