    ...
    def __init__(self, version, loose):
        ...
        if not m:
            ...
            if not m.group(3):
                ...
                self.prerelease = [(int(_id) if NUMERIC.search(_id) else _id)for _id in self.prerelease]
            else:
                self.prerelease = [(int(_id) if NUMERIC.search(_id) else _id)
                                   for _id in m.group(3).split(".")]
        else:
...
def sort(_list, loose):
    keyf = loose_key_function if loose else full_key_function
    _list.sort(key=keyf)
    return list


def rsort(_list, loose):
    keyf = loose_key_function if loose else full_key_function
    _list.sort(key=keyf, reverse=True)
    return list


def gt(a, b, loose=True):
    return compare(a, b, loose) > 0


def lt(a, b, loose=True):
    return compare(a, b, loose) < 0


def eq(a, b, loose=True):
    return compare(a, b, loose) == 0


def neq(a, b, loose=True):
    return compare(a, b, loose) != 0


def gte(a, b, loose=True):
    return compare(a, b, loose) >= 0


def lte(a, b, loose=True):
    return compare(a, b, loose) <= 0


def cmp(a, op, b, loose=True):
...
def is_x(_id):
    return _id is None or _id == "" or _id.lower() == "x" or _id == "*"
