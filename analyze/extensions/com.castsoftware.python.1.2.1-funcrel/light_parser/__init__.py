...
class Walker:
    ...
    """
    def __init__(self, interpreters=None, module=None):
        
        self.interpreters = []
        
        if interpreters:
            for interpreter in interpreters:
                self.register_interpreter(interpreter(module))
...
class Node:
    ...
    def is_valid_token(self, token):
        """Check tokens are not whitespace-like
        """
        
        if token.text == "\n":
            return False
        
        if token.type in [PygmentToken.LineFeed]:#, PygmentToken.Comment]:
            return False
    ...
    def get_end_line(self):
        ret = None
        for tok in reversed(self.children):
            # recursive call
            ret = tok.get_end_line()
            
            if ret and self.is_valid_token(tok):
                return ret
...
class BlockStatement(Node):
    ...
    def print_tree(self, depth=0):
        ...
        for token in self.get_body():
            #print("[BlockStatement] token : {}".format(token))
...
class Lookahead:
    ...
    def move_to(self, tokens):
        ...
        @todo: When the stream contains a PygmentToken.Startline token,
        the loop ends. If we try to bypass this, it breaks other point
        of the program that seems to rely on this bug to correctly work.
        """
        list_text = []
